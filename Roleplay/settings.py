...
class Settings:
        ...
        async def group(ctx: commands.Context) -> None:
            if ctx.invoked_subcommand is None:
                users = await self.users_manager.list_users(
                    ctx, ctx.author, property, as_display=True
                )
    ...
    def create_toggle_command(self, property: str, values: dict) -> commands.Command:
        ...
        async def toggle(
            ...
            if state is None:
                return await ctx.send(
                    f'{member.mention} is {"" if cur_state else "not"} {get_indefinite_article(label)} {label}.'
                )

            await self.config.user(member).get_attr(property).set(state)
            return await ctx.send(
                f"{member.mention} is now {get_indefinite_article(label)} {label}."
                if state
                else f"{member.mention} is no longer {get_indefinite_article(label)} {label}."
            )
    ...
    async def settings_embed(
        ...
        for property, values in USER_SETTINGS.items():
            ...
            if data_type is list:
                users = await self.users_manager.list_users(
                    ctx, member, property, as_display=True, as_string=True
                )
