...
class Manager:
    ...
    async def in_group(
        ...
        async with self.config.user(member).get_attr(users_group)() as user_ids:
            self.logger.debug(f"{member} {users_group} user_ids: {user_ids}")
    ...
    async def add_user(
        ...
        async with self.config.user(member).get_attr(users_group)() as user_ids:
            self.logger.debug(
                f"{member.display_name} ({member.id}) {users_group} user_ids: {user_ids}"
            )
            if user_id in user_ids:
                return await ctx.send(
                    f"{target_user.display_name} is already {get_indefinite_article(label)} {label} for {member.mention}. {choice(const.INSULTS)}",
                    delete_after=const.SHORT_DELETE_TIME,
                )
                ...
                if pred.result:
                    ...
                    await ctx.send(
                        permission_accept.format(
                            target=target_user.mention, author=member.mention
                        )
                    )
                else:
                    return await ctx.send(
                        permission_deny.format(
                            target=target_user.display_name, author=member.mention
                        )
                    )
            else:
                user_ids.append(user_id)
                return await ctx.send(
                    f"{target_user.display_name} has been added as {get_indefinite_article(label)} {label} for {member.mention}.",
                    delete_after=const.SHORT_DELETE_TIME,
                )
    ...
    async def add_user_to_group(
        ...
        if user.id == member.id:
            return await ctx.send(
                f"{member.mention} tried to add themselves as {get_indefinite_article(label)} {label}. {choice(const.INSULTS)}",
                delete_after=const.SHORT_DELETE_TIME,
            )
        ...
        for excluded_group in exclusion_groups:
            ...
            if in_group:
                ...
                return await ctx.send(
                    f"{user.display_name} is {get_indefinite_article(excluded_label)} {excluded_label} for {member.mention}. They can't be added as {get_indefinite_article(label)} {label}. {choice(const.INSULTS)}!",
                    delete_after=const.SHORT_DELETE_TIME,
                )
    ...
    async def remove_user(
        ...
        async with self.config.user(member).get_attr(users_group)() as user_ids:
            self.logger.debug(f"{member.id} {users_group} user_ids: {user_ids}")
            if user_id not in user_ids:
                await ctx.send(
                    f"{user.display_name} is not in {get_indefinite_article(label)} {label} for {member.mention}. {choice(const.INSULTS)}",
                    delete_after=const.SHORT_DELETE_TIME,
                )
            else:
                ...
                label = USER_SETTINGS[users_group].get("label", "users list")
                await ctx.send(
                    f"{user.display_name} has been removed as {get_indefinite_article(label)} {label} for {member.mention}.",
                    delete_after=const.SHORT_DELETE_TIME,
                )
    ...
    async def remove_user_from_group(
        ...
        if user.id == member.id:
            return await ctx.send(
                f"{member.mention} tried to remove themselves as {get_indefinite_article(label)} {label}. {choice(const.INSULTS)}",
                delete_after=const.SHORT_DELETE_TIME,
            )
        ...
        if not in_group:
            return await ctx.send(
                f"{user.display_name} is not {get_indefinite_article(label)} {label} for {member.mention}. {choice(const.INSULTS)}",
                delete_after=const.SHORT_DELETE_TIME,
            )
    ...
    async def list_users(
        ...
        #     # Handle DM user case
        #     await ctx.send("This command currently only works in the server (guild) context.")
        #     return []
    ...
    async def get_user(self, ctx, user_key: Union[int, str]):
        ...
        if user is None:
            self.logger.debug(f'Unable to find user using "{user_key}".')
            return None
        else:
            self.logger.debug(
