...
class Marriage(commands.Cog):
    ...
    async def marryset_gg(
        ...
        if await self.config_manager.config.is_global() == make_global:
        ...
        await self.config_manager.config.clear_all_members()
        await self.config_manager.config.clear_all_users()
        await self.config_manager.config.clear_all_guilds()
        await self.config_manager.config.clear_all_globals()
        await self.config_manager.config.is_global.set(make_global)
    ...
    async def marryset_toggle(
        ...
        if await self.config_manager.config.is_global() and not self.bot.is_owner(
            ctx.author
        ):
    ...
    async def marryset_settings(self, ctx: commands.Context):
        ...
        is_global = await self.config_manager.config.is_global()
        conf = await self.config_manager.get_guild_config(ctx.guild)
        data = (
            await self.config_manager.config.all()
            if is_global
            else await self.config_manager.config.guild(ctx.guild).all()
        )
        currency_used = (
    ...
    async def marryset_actions_show(self, ctx: commands.Context, action: str):
        ...
        if not data:
            data = self.actions.DEFAULT.get(action)
        if not data:
            return await ctx.send("Uh oh, that's not a registered action.")
        await ctx.send(
            box(
...
description:: {data.get('description')}""",
    ...
    async def marryset_gifts_show(self, ctx: commands.Context, gift: str):
        ...
        if not data:
            data = self.gifts.DEFAULT.get(gift)
        if not data:
            return await ctx.send("Uh oh, that's not a registered gift.")
        await ctx.send(
            box(
...
price:: {data.get('price')}""",
    ...
    async def about(
        ...
        e.set_author(name=f"{member.name}'s Profile", icon_url=member.avatar.url)
    ...
    async def spouses(
        ...
        if not member:
            member = ctx.author
        m_conf = await self.config_manager.get_user_config()
...
{sp_text.strip()}""",
    ...
    async def perform(
        ...
        if not exertion:
            exertion = self.actions.DEFAULT.get(action)
        ...
        if member.id not in spouses and await m_conf(ctx.author).married():
            for sid in spouses:
                spouse = (
                    ...
                    if await self.config_manager.config.is_global()
    ...
    async def _is_custom(self, ctx, item):
        return self.actions.is_custom(item) or self.gifts.is_custom(item)

    async def _is_removed(self, ctx, item):
        return self.actions.is_removed(item) or self.gifts.is_removed(item)
