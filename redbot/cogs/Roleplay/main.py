...
class Roleplay(commands.Cog):
    ...
    async def interaction(
        ...
        is_blocked = await self.check_roles(ctx, invoker_member, target_member, action)
        if is_blocked:
            self.reset_cooldown(ctx, action_name)
            return False
        ...
        # If the invoker_member calling the command is in the target's owner
        is_owner = await self.user_settings.users_manager.in_group(
            target_member, invoker_member, "owners"
        )
        if is_owner:
            await self.send_action_message(
                ctx,
        ...
        if action.consent is None or action.consent is False:
            self.logger.debug(f"{action_name} consent was not required")
        else:
            has_consent = await self.get_consent(
                ctx,
                invoker_member,
                target_member,
                action,
    ...
    async def check_blocked(
        ...
        if is_blocked:
            await ctx.send(
                f"{invoker_member.mention}, you can't use that command on {target_member.display_name}."
            )
            return True
        return False
    ...
    async def get_consent(
        ...
        # the target is a bot
        if target_member.bot and invoker_member.guild_permissions.administrator:
            return True
            ...
            while len(responses) < 2:
                try:
                    ...
                    if response.author not in responses:
                        ...
                        await ctx.send(
                            f"{response.author.mention} responded with {response.content.lower()}."
                        )
